...
static const int MAX_HARTS = 256;
...
extern "C" int main(int argc, char** argv) {
    ...
    int R = 100;
    int C = 32;
    int desired_threads = 64;    // e.g., 2 cores * 16 harts/core, adjust via --T
    ...
    if (hart_in_core == 0) {   // one hart per core
    std::printf("core=%d hid=%d &g_dist=%p &g_barrier_count=%p\n",
                core_id, hid, (void*)g_dist, (void*)&g_barrier_count);
	}
	...
	auto check = [&](int r, int c) {
    	...
    	bool ok2 = true;
    	ok2 &= check(0,0);
    	ok2 &= check(g_R-1, g_C-1);
    	ok2 &= check(g_R/2, g_C/2);
    	ok2 &= check(g_R-1, 0);
    	ok2 &= check(0, g_C-1);

    	std::printf(ok2 ? "DIST_CHECK_PASS\n" : "DIST_CHECK_FAIL\n");

}

barrier(total_threads);

// Everyone sees the decision
if (g_bfs_done) {
    return 1;
}
