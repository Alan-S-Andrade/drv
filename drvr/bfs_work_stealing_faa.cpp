// Level-synchronous BFS with per-core work stealing — FETCH-AND-ADD variant
// Replaces CAS-based queue pop/steal with lighter FAA-based reservation.
...
// - Queue ops use atomic_fetch_add instead of compare_and_swap for lower latency
...
// Try to acquire the thief token via swap (lighter than CAS).  Returns true if acquired.
...
// Linearizable pop/steal: reserve index by FAA on head (no retry loop)
static inline int64_t queue_pop_fifo(WorkQueue* q, int core_id) {
    // FAA unconditionally reserves a slot; check validity after
    int64_t old_h = atomic_fetch_add_i64(&q->head, 1);
    int64_t t = atomic_load_i64(&q->tail);

    if (old_h >= t) {
        // Over-reserved: no valid item at this index
        core_has_work[core_id] = 0; // best-effort empty hint
        return -1;
    }

    return frontier_current_get(q->start_idx + old_h);
}

// Batch pop: reserve up to max_chunk items from head in one FAA.
// Returns count of items claimed (0 if empty). Sets *out_begin.
...
static void process_bfs_level(int tid, int32_t level) {
    ...
    while (g_level_remaining.load(std::memory_order_acquire) > 0) {
        ...
        if (count > 0) {
            ...
            // FAA-based pop returned 0 — queue is empty (no CAS contention possible).
...
static void bfs() {
    ...
    if (tid == 0) {
        ...
        std::printf("=== BFS with Work Stealing (FAA queues + global remaining) ===\n");
