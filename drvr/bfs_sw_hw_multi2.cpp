...
extern "C" int main(int argc, char** argv) {
    ...
    if (tid_global < 0 || tid_global >= total_threads) {
        // Wait until the master says we are done
        while (atomic_load_i32(&g_program_done) == 0) {
            hartsleep(1000); // Sleep briefly to save bandwidth, then check again
        }
 ...
 //   if (tid_global == 0) { std::printf("DBG: after reductions barrier\n"); std::fflush(stdout); }
    
    if (tid_global == 0) { 
        std::printf("DBG: after reductions barrier\n"); 
        std::fflush(stdout); 
        
        // Tell the 48 parked threads to go home
