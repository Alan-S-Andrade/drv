...
static inline int get_thread_id() {
    return myCoreId() * g_harts_per_core + myThreadId();
}
...
static constexpr int DISC_BUF_SIZE = 32;  // per-hart discovery buffer for batching
...
static inline void flush_disc_buf(int64_t* disc_buf, int& disc_count) {
    if (disc_count <= 0) return;
    int64_t base = atomic_fetch_add_i64(&g_next_frontier.tail, (int64_t)disc_count);
    int32_t my_core = myCoreId();
    if (base + disc_count > g_next_frontier.capacity) {
        std::printf("ERROR: next frontier overflow (base=%ld count=%d cap=%ld)\n",
                    (long)base, disc_count, (long)g_next_frontier.capacity);
        std::abort();
    }
    for (int i = 0; i < disc_count; i++) {
        frontier_next_item_set(base + i, disc_buf[i]);
        frontier_next_owner_set(base + i, my_core);
    }
    disc_count = 0;
}
...
static void compute_hart_bfs_level(int tid, int32_t level) {
    ...
    int64_t local_discovered = 0;

    int64_t local_buf[BFS_CHUNK_SIZE];
    int64_t stolen_buf[STEAL_K];

    // Discovery buffer for batched next-frontier pushes
    int64_t disc_buf[DISC_BUF_SIZE];
    int disc_count = 0;
    ...
    while (g_level_remaining.load(std::memory_order_acquire) > 0) {
        ...
        if (count > 0) {
            for (int64_t i = 0; i < count; i++) {
                local_processed++;
                local_l1_pops++;
                process_single_node(local_buf[i], level, local_edges,
                                    disc_buf, disc_count, local_discovered);
            }
            // Batch decrement g_level_remaining for the whole chunk
        ...
        // Tier 2: Pop directly from L2SP queue (FAA-based fallback)
        {
            ...
            if (count > 0) {
                for (int64_t i = 0; i < count; i++) {
                    ...
                    local_processed++;
                    local_l2_pops++;
                    process_single_node(u, level, local_edges,
                                        disc_buf, disc_count, local_discovered);
                }
                g_level_remaining.fetch_sub(count, std::memory_order_acq_rel);
                empty_streak = 0;
        ...
        // Flush pending discoveries before waiting
        flush_disc_buf(disc_buf, disc_count);
            ...
            for (int k = 0; k < STEAL_VICTIMS; k++) {
                ...
                if (count > 0) {
                    ...
                    for (int64_t i = 0; i < count; i++) {
                        local_processed++;
                        local_stolen_items++;
                        process_single_node(stolen_buf[i], level, local_edges,
                                            disc_buf, disc_count, local_discovered);
                    }
                    g_level_remaining.fetch_sub(count, std::memory_order_acq_rel);
                    empty_streak = 0;
    ...
    // Final flush of any remaining discoveries
    flush_disc_buf(disc_buf, disc_count);

    // Write back local stats (one atomic per counter, not per node)
    ...
    atomic_fetch_add_i64((volatile int64_t *)&stat_edges_traversed, local_edges);
    atomic_fetch_add_i64(&discovered, local_discovered);
}
...
// ────────────────────── Level Advance ──────────────────────
// Total nodes to copy is stored here so all harts can read it after the barrier.
__l2sp__ volatile int64_t g_advance_total_nodes = 0;

static void advance_to_next_level_balanced(int tid) {
    const int total_harts = g_total_harts;

    // Hart 0: partition work into per-core queues
    if (tid == 0) {
        int total_cores = g_total_cores;
        int64_t total_nodes = g_next_frontier.tail;
        g_advance_total_nodes = total_nodes;
    ...
    // All harts: parallel copy of next frontier → current frontier
    int64_t total_nodes = g_advance_total_nodes;
    if (total_nodes > 0) {
        int64_t per_hart = total_nodes / total_harts;
        int64_t rem = total_nodes % total_harts;
        int64_t my_start = tid * per_hart + (tid < rem ? tid : rem);
        int64_t my_count = per_hart + (tid < rem ? 1 : 0);

        for (int64_t i = 0; i < my_count; i++) {
            frontier_current_set(my_start + i, frontier_next_item_get(my_start + i));
        }
    }
...
static void bfs() {
    ...
    // L1SP layout: stacks are allocated top-down by the runtime.
    //   Hart 0 stack: [L1SP_size - stack_per_hart, L1SP_size)  (top)
    //   Hart 1 stack: [L1SP_size - 2*stack_per_hart, L1SP_size - stack_per_hart)
    //   ...
    //   Hart N-1 stack: [0, stack_per_hart)                     (bottom)
    // The L1SP queue is placed at the bottom (offset 0+), growing upward.
    // It shares space with the bottom hart(s)' stacks, which grow downward
    // from the top of their region.  We must not let queue data overlap
    // with the portion of those stacks that is actually used.
    if (myThreadId() == 0) {
        thief_token_init();
        const uint64_t l1sp_total = g_core_l1sp_bytes;
        const int hpc = g_harts_per_core;
        const uint64_t stack_per_hart = l1sp_total / (uint64_t)hpc;

        // Reserve enough stack space per hart in the queue region.
        // The queue must fit below (stack_per_hart - STACK_RESERVE) in the
        // bottom hart's region.  If it can spill into the next hart's region,
        // reserve STACK_RESERVE there too, etc.
        static constexpr uint64_t STACK_RESERVE = 4096; // 4KB minimum per hart

        // Max usable offset for queue items: bottom hart's unused portion
        int64_t max_queue_end = (int64_t)stack_per_hart - (int64_t)STACK_RESERVE;
        int64_t queue_bytes = max_queue_end - (int64_t)L1_QUEUE_ITEMS_OFFSET;
        ...
        if (tid == 0) {
            std::printf("L1SP queue: stack_per_hart=%lu, max_queue_end=%ld, "
                        "capacity=%ld items (%ld bytes)\n",
                        (unsigned long)stack_per_hart, (long)max_queue_end,
                        (long)capacity, (long)(capacity * (int64_t)sizeof(int64_t)));
        }
    }
...
__l2sp__ volatile int g_sim_exit = 0;
...
extern "C" int main(int argc, char** argv) {
    ...
    if (tid == 0) {
        ...
        // Wait until g_total_harts is set so get_thread_id() works correctly
        while (g_total_harts == 0) hartsleep(10);
    }
