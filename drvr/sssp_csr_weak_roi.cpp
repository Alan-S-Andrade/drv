// sssp_csr_weak_roi.cpp
// ROI-only CSR graph SSSP: skips Phase 1 (parallel init) by bulk-loading
// pre-generated dist[] and frontier bitmaps. Only simulates Phase 2 (SSSP loop).
//
// Pre-initialized files (generated by gen_sssp_init_state.py):
//   sssp_dist_init.bin     - dist[] array (N int32, all INT32_MAX except source=0)
//   sssp_frontier_init.bin - frontier + next_frontier bitmaps (2 * bm_words int32)
//
// Memory layout:
...
static const int32_t SSSP_INF = 0x7FFFFFFF;
...
// SSSP control
__l2sp__ volatile int g_sssp_done;
__l2sp__ volatile int g_sim_exit;
__l2sp__ int32_t g_sssp_iters;
...
__l2sp__ int32_t g_source;             // SSSP source vertex
...
extern "C" int main(int argc, char **argv)
{
    ...
    // ================================================================
    // Phase 0: Thread 0 bulk-loads graph + pre-initialized SSSP state
    // ================================================================
    ph_stat_phase(0);
    barrier.sync([&]() {
        ...
        // Bulk load pre-initialized dist[] (all INT32_MAX, source=0)
        bulk_load("sssp_dist_init.bin", g_dist, dist_bytes);

        // Bulk load pre-initialized frontier + next_frontier bitmaps into L2SP
        size_t bitmaps_bytes = 2 * (size_t)bm_words * sizeof(int32_t);
        bulk_load("sssp_frontier_init.bin", (void *)g_frontier, bitmaps_bytes);

        std::printf("ROI-only SSSP: Phase 1 skipped (bulk-loaded init state)\n");
        std::printf("dist[]: DRAM (%lu bytes)\n", (unsigned long)dist_bytes);
        std::printf("CSR SSSP (bulk load): N=%d E=%d degree=%d source=%d\n",
    ...
    if (tid == 0)
        std::printf("SSSP from source %d\n", source);

    // ================================================================
    // Phase 2: Main SSSP loop (level-synchronous, unit weights) — THE ROI
    // ================================================================
    uint64_t t_sssp_start = cycle();
    ...
    while (true) {
        ...
        for (int w = w_lo; w < w_hi; ++w) {
            ...
            while (word != 0) {
                ...
                for (int ei = edge_begin; ei < edge_end; ++ei) {
                    ...
                    // Atomic CAS-loop: relaxation with atomic min
                    int old_d = local_dist[u];
                    while (new_d < old_d) {
                        int prev = atomic_compare_and_swap_i32(&local_dist[u], old_d, new_d);
                        if (prev == old_d) {
                            // Successfully relaxed — add to next frontier
                            int wi = u / 32;
                            int32_t mask = 1 << (u % 32);
        ...
        if (g_sssp_done) break;
    }

    uint64_t t_sssp_end = cycle();
    ...
    // ================================================================
    if (tid == 0) {
        uint64_t sssp_cycles = t_sssp_end - t_sssp_start;

        std::printf("SSSP done in %d iterations (%llu cycles)\n",
                    g_sssp_iters, (unsigned long long)sssp_cycles);
        ...
        std::printf("SSSP complete, signaling exit.\n");
        std::fflush(stdout);
