// Level-synchronous BFS with tiered work queues (L1SP local + L2SP shared)
// - Per-core L1SP work queue: fast (~1 cycle), accessed by compute harts
// - Per-core L2SP shared work queue: medium (~10 cycles), work redistribution target
...
static constexpr const char* BENCH_TITLE = "BFS with Tiered Work Queues (L1SP + L2SP)";
static constexpr const char* BENCH_LOG_PREFIX = "run_tiered_wq";
...
struct L1QueueHeader {
    volatile int64_t head;          // consumer index (CAS by compute harts)
...
// Pop a batch from L1 queue (used by compute harts)
// Uses CAS on head for thread safety among compute harts
static inline int64_t l1_queue_pop_chunk(int64_t* out, int64_t max_chunk) {
    ...
    while (true) {
        ...
        int64_t new_h = h + chunk;

        if (atomic_compare_and_swap_i64(&hdr->head, h, new_h) == h) {
...
// Push a batch to L1 queue (used ONLY by fetcher hart - no CAS needed on tail)
...
// Trace infrastructure (reused from baseline)
...
// Dedicated to pulling work from L2SP queue into L1SP queue.
...
static void fetcher_loop(int tid, int32_t level) {
    ...
    L1QueueHeader* l1hdr = l1_queue_header_ptr();
    ...
    while (g_level_remaining.load(std::memory_order_acquire) > 0) {
        ...
        if (l1_depth < FETCHER_LOW_WATERMARK) {
            ...
            int64_t got = queue_pop_chunk(my_queue, my_core, &begin_idx, fetch_count);
            if (got > 0) {
                ...
                if (pushed < got) {
                    // L1 queue full - this shouldn't happen often with watermarks
                    // but handle gracefully by pushing remainder back to
                    // the next frontier (they'll be redistributed next level)
                    // For simplicity, just track un-pushed items as lost
                    // (they are already consumed from L2SP queue)
                    // In practice the watermarks prevent this
                }
            ...
            if (total_cores > 1 && thief_token_try_acquire()) {
                ...
                for (int k = 0; k < STEAL_VICTIMS; k++) {
                    ...
                    begin_idx = 0;
                    got = queue_pop_chunk(&core_queues[victim], victim, &begin_idx, STEAL_K);
                    if (got > 0) {
                        found = true;
                        for (int64_t i = 0; i < got; i++) {
...
// Harts 1..N-1 process BFS nodes using tiered queues:
//   1. Pop from L1SP queue (fast, ~1 cycle)
//   2. If L1SP empty, pop directly from L2SP queue (fallback)
...
static void compute_hart_bfs_level(int tid, int32_t level) {
    ...
    while (g_level_remaining.load(std::memory_order_acquire) > 0) {
        // Tier 1: Pop from L1SP queue (fast path)
        ...
        // Tier 2: Pop directly from L2SP queue (fetcher might be slow/busy)
        {
            int64_t begin_idx = 0;
            count = queue_pop_chunk(my_queue, my_core, &begin_idx, BFS_CHUNK_SIZE);
            if (count > 0) {
                for (int64_t i = 0; i < count; i++) {
...
static void bfs() {
    ...
    if (myThreadId() == 0) {
        thief_token_init();
        // Calculate L1SP queue capacity
        // Reserve space for: thief token (64 bytes), header, guard for stacks
    ...
    if (tid == 0) {
        ...
        std::printf("TIERED_STATS: l1_pops=%ld l2_pops=%ld stolen=%ld fetch_ops=%ld fetch_items=%ld\n",
