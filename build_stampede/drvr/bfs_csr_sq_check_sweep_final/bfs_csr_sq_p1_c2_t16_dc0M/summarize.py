...
def main():
    ...
    # DRAM MemCtrl-side outstanding (true DRAM-side queue depth, includes atomics+stores)
    # Workaround for hart-side dram_outstanding_requests_ only counting LOAD ops.
    dram_mc_for_oq = {k: v for k, v in memctrl_stats.items()
                      if "dram" in k and "l2sp" not in k and "l1sp" not in k}
    if dram_mc_for_oq:
        per_slice_avg = []
        per_slice_max = []
        sum_avg = 0.0
        sum_max = 0
        for k in sorted(dram_mc_for_oq.keys()):
            oq = dram_mc_for_oq[k]["outstanding_requests"]
            avg_q = oq["sum"] / oq["count"] if oq["count"] > 0 else 0
            max_q = oq["max"]
            per_slice_avg.append((short_memctrl_name(k), avg_q))
            per_slice_max.append((short_memctrl_name(k), max_q))
            sum_avg += avg_q
            sum_max += max_q
        print(f"\n{'--- DRAM MemCtrl-side Outstanding (total phase, all req types) ---':<45}")
        print(f"{'  DRAM Outstanding SUM (avg across slices)':<45} {sum_avg:.2f}")
        print(f"{'  DRAM Outstanding SUM (max across slices)':<45} {sum_max}")
        for name, v in per_slice_avg:
            print(f"{'    ' + name + ' avg':<45} {v:.2f}")
        for name, v in per_slice_max:
            print(f"{'    ' + name + ' max':<45} {v}")
