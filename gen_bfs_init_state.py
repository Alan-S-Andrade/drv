#!/usr/bin/env python3
"""Pre-generate initialized BFS data structures for ROI-only simulation.

Reads uniform_graph.bin header to extract N and source vertex, then generates:
  - bfs_dist_init.bin:     N int32 values, all -1 except dist[source] = 0
  - bfs_frontier_init.bin: ceil(N/32) int32 words for frontier (bit set for source),
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate pre-initialized BFS state for ROI-only simulation")
    ...
    # Generate dist[] init: all -1, except dist[source] = 0
    dist = array.array("i", [-1] * N)
    dist[source] = 0

    dist_path = f"{args.output_dir}/bfs_dist_init.bin"
    ...
    frontier_path = f"{args.output_dir}/bfs_frontier_init.bin"
