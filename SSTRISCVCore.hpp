...
public:
    ...
    struct ThreadStats {
        ...
        Statistic<uint64_t> *total_instructions;
        uint64_t total_instructions_count = 0;
    };

    struct BasicBlockRuntimeState {
        bool active = false;
        uint64_t start_pc = 0;
        uint64_t last_pc = 0;
        uint64_t instruction_count = 0;
        uint64_t arithmetic_count = 0;
    };

    struct BasicBlockAggregateStats {
        uint64_t visits = 0;
        uint64_t total_instructions = 0;
        uint64_t total_arithmetic = 0;
    };
...
#include <InstructionTable.h>
            ...
            {"total_instructions", "Total instructions executed", "count", 1},
...
#undef DEFINE_DRV_STAT
    ...
    std::vector<BasicBlockRuntimeState> basic_block_runtime_;
    std::map<std::pair<uint64_t, uint64_t>, BasicBlockAggregateStats> basic_block_aggregate_;
};
