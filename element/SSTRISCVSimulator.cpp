...
/**
 * @brief Handle MMIO_BULK_LOAD: read a file from host and memcpy directly
 *        to the memory backing store, bypassing the memory hierarchy / Ramulator.
 *
 * The descriptor is a struct in simulated memory:
 *   int64_t filename_addr  — pointer to null-terminated filename string
 *   int64_t dest_addr      — destination address in simulated memory
 *   int64_t size           — number of bytes to load
 *   int64_t result         — output: bytes loaded, or -1 on error
 */
void RISCVSimulator::handleBulkLoad(RISCVSimHart &shart, uint64_t desc_addr) {
    struct BulkLoadDesc {
        int64_t filename_addr;
        int64_t dest_addr;
        int64_t size;
        int64_t result;
    };
